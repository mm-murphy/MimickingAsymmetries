...
# utest = changed uniform limb rprs to 'this_rprs1'
# utest2 = changed scatter from 100 to 10
# utest3 = changed scatter from 100 to 750
...
scatter = 750. # [ppm], standard deviation of flux values about the model
output_file_path = './output_files/wasp39b_utest3_synth_scatter'+str(int(scatter))+'_ahead'+str(int(Ntransits_ahead))+'LDscenC_nircCadence.txt'
figure_output_path = './figures/wasp39b_utest3_synth_scatter'+str(int(scatter))+'_ahead'+str(int(Ntransits_ahead))+'LDscenC_nircCadence/'
array_output_path = './output_arrays/wasp39b_utest3_synth_scatter'+str(int(scatter))+'_ahead'+str(int(Ntransits_ahead))+'LDscenC_nircCadence'
...
lit_params = {
    ...
    't0':np.array([2455342.96913, 0.00063, 'days', 'Mancici+ 2018'],dtype=object),
    'P':np.array([4.0552941, 0.0000034, 'days', 'Mancici+ 2018'], dtype=object),
    'log10P':np.array([np.log10(4.0552941), ((c*0.0000034)/4.0552941), 'unitless', 'calculated'], dtype=object),
    'a':np.array([11.55, 0.13, 'Rs', 'Fischer+ 2017'], dtype=object),
    'log10a':np.array([np.log10(11.55), ((c*0.13)/ 11.55), 'unitless', 'calculated'], dtype=object),
    'RpRs':np.array([0.1457, 0.0015, 'unitless', 'Maciejewski 2016'], dtype=object),
    #'RpRs':np.array([0.1158, 0.0017, 'unitless', 'pre determined for this band'], dtype=object),
    'Rs':np.array([0.92868, 0.03, 'Rsun', 'Gaia DR2'], dtype='object'),
    'Mp':np.array([0.281, 0.032, 'Mjupiter', 'Mancini+ 2018'], dtype='object'),
    'Teq':np.array([1166, 14., 'K', 'Mancini+ 2018'], dtype='object'),
    'inc':np.array([87.32, 0.17, 'degrees', 'Mancini+ 2018'], dtype=object),
    'cosi':np.array([np.cos(87.32*(np.pi/180.)), np.sin(87.32*(np.pi/180.))*(0.17*(np.pi/180.)), 'unitless', 'calculated'], dtype=object),
    ...
    'T14':np.array([2.8032, 0.0192, 'hours', 'Faedi+ 2011'], dtype=object)
}
...
for i_factor, asym_factor in enumerate(asymmetry_factors_totest):
    ...
    rprs_homog = np.sqrt(np.mean([this_rprs1**2, this_rprs2**2])) # this_rprs1
